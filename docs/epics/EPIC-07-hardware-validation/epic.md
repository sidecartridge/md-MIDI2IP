...
- Out of scope: the automated regression gate — **moved to Iteration 2
  (EPIC-10 Hardware validation II)**, since gating is premature while the transport
  is being redesigned (D-12); the test peer itself — that's the **Hatari gateway
...
STORY-01 delivered the measurement — and its result is the single most important
