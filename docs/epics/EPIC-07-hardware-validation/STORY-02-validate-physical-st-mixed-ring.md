...
- [x] A physical Atari ST running MIDI Maze (via its RP) and a second node in Hatari both connect to the orchestrator, forming a mixed hardware/software ring
...
- [x] **Game launch does NOT work yet**: START-GAME / the ~4 KB `SEND-DATA` maze can't complete over the RP-hardware path; the D-12 throughput ceiling is the boundary this story documents

## Acceptance

On real hardware (physical ST + a Hatari node through the orchestrator) the ring
forms, the nodes elect master/slave, and user names exchange. **Launching the
