...
ring traffic. Hardware testing surfaced two **protocol-level** blockers a
...
- **Out:** game-session UI/stats ("gameplays disputing/disputed", D-10) beyond
  what the model needs; keeping the RP/byte-pipe dumb (D-02), since **all**
  protocol awareness lives in the orchestrator; drones / >2 players (post-MVP, D-04).
...
- The full 2-player **hardware** match (the old STORY-04) **moved to EPIC-10
  STORY-02** (Iteration 2): it is not blocked by the coordination logic but by the
