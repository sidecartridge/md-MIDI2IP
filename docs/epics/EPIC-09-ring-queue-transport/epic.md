...
shared-memory ring** (RP→ST), each side owning its own index with no token wait,
...
boot/config menu, MIDI runs after the firmware launches into GEM. The bit-8/9
markers therefore never collide with a live command payload word (a mode flag can
enforce it if ever needed).
...
loop re-read the same byte ~13× (`IN_adv >> RX`), corrupting the ring. **Resolved
with a confirm-ack** (the documented fallback): the RP bumps `MIDI_IN_ACK` after each
...
- **In:** the OUT commemul byte-stream (m68k emit + RP consumer); the IN
  ...
  TPROTOCOL command channel (config/menu still use chandler commands, while the
  MIDI byte stream is a *second* consumer on the same sample stream); flow-control /
...
- Bidirectional, byte-oriented, **opaque** (no MIDI parsing; order preserved per D-02).
