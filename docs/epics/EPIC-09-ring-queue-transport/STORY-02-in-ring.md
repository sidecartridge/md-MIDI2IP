...
status; `Bconin(3)` reads the head and fires a one-cycle bit-9 "advance" signal, with
no read index on the m68k and no command.

## Tasks

- [x] RP: an IN ring (16 KB) fed by the network; proactively publish the head byte (`MIDI_IN_BUFFER`, byte replicated 4× for endian safety) and a pre-baked Bconstat status (`MIDI_IN_STATUS`, -1/0) into the shared region on every change
...
orchestrator) byte-exact and in order, with no per-byte command. IN throughput is well
...
many times (`IN_adv` ≈ 13× `RX`), corrupting the ring and producing "too many machines" / glitches.
