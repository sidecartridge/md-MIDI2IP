...
- [x] Mirror for OUT: a 16 KB OUT ring filled from the hot path, drained to TCP in the poll context up to `tcp_sndbuf` (retries instead of dropping), fixing the burst `tcp_write` drop (`OUT > RX`).
