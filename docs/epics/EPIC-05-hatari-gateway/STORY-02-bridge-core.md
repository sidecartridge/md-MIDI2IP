...
- [x] Reads the orchestrator socket and `os.write`s to the Atari-IN fifo (the `CMD_MIDI_RECV` / Bconin analogue)
...
Mirrors `midi.c`: OUT fifo = the m68k OUT path, IN fifo = the RP IN queue → Bconin.
