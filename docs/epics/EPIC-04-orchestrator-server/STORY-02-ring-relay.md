...
- [x] Ring re-forms on join/leave (`next_player` computed fresh each chunk); **a lone node has no peer — no self-echo** (it must not elect/count before the ring forms, D-04), 2 players = A↔B
