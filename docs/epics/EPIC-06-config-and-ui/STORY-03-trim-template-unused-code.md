...
Closed as sufficient: removed the SD-card subsystem and the entire settings-command machinery (emul.c handlers/table + term.c `term_cmd*` impls and parse helpers + term.h decls), covering the bulk of the unused template. UF2 went 912->814 KB. Minor leftovers (the unused `FOLDER` config key, demo strings in `userfw.s`) were deemed not worth further churn.
