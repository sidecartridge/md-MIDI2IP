...
- [ ] Remove the extra parameters from the status/menu screen, keeping only the three below
- [ ] Show **Wi-Fi status** (down / associating / IP-acquired)
- [ ] Show the device's **local IP address** (`network_getCurrentIp`)
- [ ] Show the **orchestrator connection status** (down / connecting / up — from the `midi_net_*` state machine, `midi_net_status_str`)
- [ ] Render on both the Atari framebuffer and local OLED paths, refreshing live
