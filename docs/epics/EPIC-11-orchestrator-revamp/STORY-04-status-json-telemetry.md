...
- [x] Include per-node `connected_s` (up-time) and `idle_s` (seconds since last byte — lets the UI dim a stalled node; replaces STORY-02's dropped stale-flush count)
