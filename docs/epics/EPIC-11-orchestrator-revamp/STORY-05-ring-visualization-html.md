...
- [ ] Serve a self-contained page (HTML/CSS/JS, no external/CDN deps) at `/`
- [ ] Draw the ring: nodes placed around a circle with the relay direction indicated
- [ ] Poll `/status.json` every 2 s and render each node's IP/host + bytes in/out as an overlay at its ring position
- [ ] Handle 0 / 1 / N nodes gracefully (empty ring, single node, re-layout on join/leave)
- [ ] Show connection/stale state visually (e.g. dim a stalled node) and degrade gracefully if `status.json` is unreachable
